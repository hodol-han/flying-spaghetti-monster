      ...
      {combinations.length === 0 ? (
        ...
        <div className="space-y-4">
          {combinations.map((item, index) => (
            <div key={index} className="p-4 border border-gray-200 rounded-lg hover:border-secondary transition-all">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm text-gray-500">조합 #{index + 1}</span>
                <span className="text-2xl" title={`평점: ${item.rating}/5`}>{getRatingEmoji(item.rating)}</span>
              </div>
